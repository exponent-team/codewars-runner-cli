...
function runMocha(opts, interfaceType, runCode, file) {
  var args = ['/.npm-global/bin/mocha', '-t', opts.timeout || 8000, '-u', interfaceType, '-R', 'mocha-reporter', file];
...
function runKarma(opts, runCode, fail, interfaceType, config) {
  ...
  let sConfig = JSON.stringify(config, null, 4)
...
function createNodeRunCommand(args, opts) {
  var version = versionInfo(opts);
  var nodeCmd = `/usr/local/n/versions/node/${version.node}/bin/node`;
...
// Simplified version info for specifically node 22.15.0
